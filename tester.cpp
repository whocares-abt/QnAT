...
int main(){
    ...
    std::string corpus;
    cout << "Choose the corpus: \"Mahatma Gandhi\" or \"Raman Maharshi\"" << endl;
    getline(cin, corpus);

    if ((corpus != "Mahatma Gandhi") && (corpus != "Raman Maharshi")){
        throw std::runtime_error("Incorrect input.");
    }

    QNA_tool qna_tool(corpus);

    if (corpus == "Mahatma Gandhi"){
        int num_books = 98;
        ...
        for(int i = 1; i <= num_books; i++){
            ...
            if (!inputFile.is_open()) {
                std::cerr << "Error: Unable to open the input file raman maharshi." << std::endl;
                return 1;
            }
    ...
    */
    std::string query; std::string mode;
    std::ifstream filename("question.txt");

    getline(std::cin, mode);

    if (mode == "terminal"){
        while (getline(std::cin, query)){
            if (query == "close"){break;}
            qna_tool.query(query,"api_call.py");
        }
    }
    else if (mode == "file"){
        while (getline(filename, query)){
            cout << "Learn the following text" << endl << endl;
            cout << "\"\"\" " << endl;
            qna_tool.query(query,"api_call.py");
        }
    }  
